...
- [x] Crypto 历史成交量快照重建：[build_point_in_time_crypto_perpetual](quantbench/data/universe.py) 从累积的每日快照（[record_crypto_universe_snapshot](quantbench/data/warehouse.py)）重建 membership intervals，快照覆盖不到的日期诚实报错而非静默假装 PIT。**代码已就绪、诚实标注数据待积累**：快照从 `quantbench universe snapshot-crypto` 首次运行起才开始累积，窗口越早、区间越稀疏——这是数据时效问题，不是代码缺口。
...
**缺什么**：
- [x] Crypto 最低成本方案已落地：`quantbench universe snapshot-crypto` 每日快照当前 top-N 排名（[warehouse.py](quantbench/data/warehouse.py) 的 `record_crypto_universe_snapshot`），随时间自然积累无偏差数据，且已能被 [build_point_in_time_crypto_perpetual](quantbench/data/universe.py) 消费重建 PIT universe。**未做**：交易所归档/Tardis 等回溯型历史数据源。
- [ ] Equity：评估含退市股票的数据源（见 1.3）——**仍需付费源**（Polygon 等），本阶段不做，已通过下面的能力标记诚实反映。
- [x] 在 universe 元数据中记录"本 universe 覆盖退市标的：是/否"，Reviewer 对"否"的 PIT 截面 run 输出结构性警告并影响 verdict 上限（例如封顶 PROMISING，不给 STRONG）。**已从硬编码升级为自动传播**：[ProviderResult.covers_delisted](quantbench/data/providers/base.py) 由 provider 自己声明该能力，[fetch_universe_ohlcv](quantbench/data/warehouse.py) 聚合所有已拉取 symbol 的声明（任一 symbol 未覆盖或拉取失败即视为不覆盖），自动写回 `UniverseDefinition.covers_delisted`，不再是永远 `False` 的死字段。yfinance/ccxt_perpetual 如实声明 `False`；Polygon stub 注释了启用后应设 `True`。
...
- [x] **接口预留（PHASE13B §1.6）**：`ExecutionBackend` 抽象（`local` 实装 / `remote` stub），`screen_factors` fan-out 经其派发；切 `remote` 抛清晰 `NotImplementedError`，不静默回退。实际远程执行（SSH/Modal）仍待真实瓶颈出现再实装。
...
**缺什么**：
- [x] 标准化信号导出：`quantbench factor export <name>`（[signal_export.py](quantbench/factors/signal_export.py)）给定截面因子，输出当期目标权重的 JSON（含时间戳、因子版本 hash）。**v1 仅覆盖截面因子**——"因子 + universe"对单资产因子不natural成立，单资产导出留后续增量。
- [x] 导出内容附带完整溯源引用（来自哪个 run、什么 verdict、lifecycle 状态、已知局限 `source_findings`），"可审计"延伸到交接面。

### 5.4 成本与用量可观测性（优先级：🟠 低）

- [x] 每次 run 记录 LLM token 用量与成本，写入 manifest `llm_usage` 段（复用 litellm 自带 `completion_cost`，不手搓定价表；Critic 和记忆固化 SubAgent 的开销也纳入，此前完全不可见）。数据 API 调用次数已由既有 `mcp_calls`/`data_slices` 间接覆盖；执行耗时分解未做，`duration_seconds` 仍是唯一的整体耗时字段。
- [x] 批量筛选（screen_factors）执行前给出成本预估：返回 payload 的 `cost_estimate` 字段（复用 1.4 审查台的 `CostEstimate`），仅做可见、不叠加新的阻塞门（1.4 已有的 per-candidate StagingGate 继续独立生效）。
