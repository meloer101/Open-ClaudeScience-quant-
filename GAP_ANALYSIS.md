...
**现状**：`run_signal_code`（`codeexec.py`）已把 `compute()` 的执行搬进 `resource.setrlimit` 约束的独立子进程（`skills/sandbox.py`），不再是同进程 `exec()`。CPU 时间与父进程 wall-clock 兜底在 Linux/macOS 均生效；`RLIMIT_AS` 内存上限在 Linux 上生效，在 macOS/Darwin 上因内核限制无法通过 `setrlimit` 设置，做了显式降级（不静默假装生效）。仍未做 Docker 级的文件系统/网络隔离。

**缺什么**：
- [x] 受限子进程 + rlimit 沙箱执行模型生成代码（`run_signal_code` 路径）；结构化超限错误，run 不崩溃。
- [x] 超时（wall-clock 兜底）/CPU 上限；内存上限（Linux 生效，macOS 已知限制并显式探测降级）。
- [ ] 文件系统访问白名单（当前靠 builtins 黑名单禁 `open`/`__import__` 实现"实质等价"，未做真正的只读/只写目录挂载）。
- [ ] 交叉截面/`screen_factors` 路径（经 `load_signal_function` 返回的 `compute` 被引擎按 symbol 逐个调用）尚未接入沙箱——按 symbol 粒度沙箱化会引入显著的子进程开销，需要先决定是"整个 panel 一次沙箱化"还是接受当前粒度，留作后续增量。
