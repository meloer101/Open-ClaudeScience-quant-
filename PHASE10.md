...
#### 2.4.1 CPCV + purge/embargo（阶段内 stretch）

- [ ] 组合式покрытие（Combinatorial Purged CV）+ purge/embargo，防止信号 lookback 导致训练/测试边界信息泄漏。`embargo_bars` / `purge_bars` 与因子 lookback 联动——从 `compute()` 的 rolling window 参数推断，或要求 config 显式声明 `lookback_bars`。
...
7. **CPCV/purge（2.4.1）** — stretch，可延后到 Phase 11。
...
- **不做贝叶斯 / 因子模型级的显著性**（IC 的 Newey-West t 统计留作 2.5 的加分项，非必做）。
